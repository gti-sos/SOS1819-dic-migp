    ...
    ["$scope", "$http", "$httpParamSerializer", function($scope, $http, $httpParamSerializer){
        ...
        $http.get(API).then(function(response) {
            $http.get(ExternAPI).then(function(response2) {
                ...
                var ESrate = response.data.filter(c => c.country=="Spain" && c.year==2018)[0].density;
                var FR = paises.filter(c =>  c=="FR");
                var FRrate = response.data.filter(c => c.country=="France" && c.year==2018)[0].density;
                var GR = paises.filter(c =>  c=="DE");
                var GRrate = response.data.filter(c => c.country=="Germany" && c.year==2018)[0].density;
                var US = paises.filter(c =>  c=="US");
                var USrate = response.data.filter(c => c.country=="United States" && c.year==2018)[0].density;
                var UK = paises.filter(c =>  c=="GB");
                var UKrate = response.data.filter(c => c.country=="United Kingdom" && c.year==2018)[0].density;
                ...
                function drawMultSeries() {
                  var data = google.visualization.arrayToDataTable([
                    ['País', 'Puntos de alquiler de bicicletas', 'Densidad'],
                  ...
                  var options = {
                    title: 'Número de puntos de alquiler de bicicletas y densidad',
                    chartArea: {width: '50%'},
                    hAxis: {
                      title: '',
                      minValue: 0
